        ...
        function fetchInternships(city, industry, duration) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    results.empty();
                    if (data.length > 0) {
                        var resultHeader = $('<h3>').text(
                            `Start dates in ${city} for ${industry}`);
                        ...
                        $.each(data, function (index, internship) {
                            ...
                            var applyLink = $(
                                '<button href="javascript:void(0);" onclick="applyNow(' +
                                internship.id + ')">Apply Now</button>').addClass(
                                'apply-button');
                            var row = $('<div>').addClass('internship-row');

                            var dateColumn = $('<div>').addClass('date-column').html(
                                '<h4>' + formatDate(internship.start_date) +
                                '</h4><p>2024</p>');
                            var priceColumn = $('<div>').addClass('price-column').text(
                                totalCost.toFixed(2) + ' ' + selectedCurrency); // Display selected currency
                            var availabilityColumn = $('<div>').addClass(
                                'availability-column').text(availability);
                            var applyColumn = $('<div>').addClass('apply-column')
                                .append(applyLink);

                            row.append(dateColumn).append(priceColumn).append(
                                availabilityColumn).append(applyColumn);
                            results.append(row);
                        });
                    } else {
                        results.append(
                            '<p>No internships found for the selected criteria.</p>');
                    }
                },
